...
v2.7.2 ベースの堅牢化:
- ビームセンサへ bounce_time を適用
- 判定中の再入防止ロック
- 1遮光サイクル（押下〜解放）につき判定1回
...
BOUNCE_TIME = 0.05     # ボタン / ビームのチャタリング防止用マスク時間 (秒)
...
# v2.7.3: ビームにも bounce_time を付与
beam_trig  = Button(12, pull_up=False, bounce_time=BOUNCE_TIME) # ビームセンサ (EX-11EB-PN: 遮光時ON)
...
def initialize_runtime():
    ...
    bind_handlers()
    logging.info("==============================================")
    logging.info(" 枚数検知監視 プログラムVer 2.7.3 (堅牢化: debounce/判定ロック/SIGTERM) ")
