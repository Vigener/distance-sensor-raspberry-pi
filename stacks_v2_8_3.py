# -*- coding: utf-8 -*-
"""
枚数検知センサシステム 制御プログラム (Ver 2.8.3)

v2.8.2 ベース + 初動・アナログ揺らぎ対策:
- 監視開始直後（およびNG自動復帰直後）の最初の N 回は判定スキップ
  （「後トリガで前を測る」方式の先頭位相ずれ・距離計立ち上がり対策）
- C-MOS を短時間複数回サンプリングし、すべて OFF のときだけ NG
  （1サンプル瞬間落ちによる誤NG抑制）
"""
...
from mode_switch_settings import conf_path, read_enable_mode_switch
...
# 初動スキップ: 監視ON直後 / NG自動復帰直後に判定しない回数
STARTUP_SKIP_COUNT = 1
# C-MOS 複数回サンプル（すべて OFF のときだけ NG）
CMOS_SAMPLE_COUNT = 3
CMOS_SAMPLE_INTERVAL = 0.01  # 秒

# 後継機向け: mode_switch.conf の enable_mode_switch で有効化
_STACKS_DIR = os.path.dirname(os.path.abspath(__file__))
ENABLE_MODE_SWITCH = False
MODE_SWITCH_CONF = str(conf_path(_STACKS_DIR))
MODE_BEEP_DURATION = 0.15  # 短いビープ（＝黄LED点灯幅）
MODE_BEEP_GAP = 0.15       # 無音／消灯ギャップ

# CSVログ保存先設定
LOG_DIR = "/home/raspai/Desktop/stacks/logs"  # ログを保存するフォルダパス
SAVE_OK_LOG = False              # OK時の判定もCSVに記録するかどうか
...
def reload_mode_switch_config():
    """mode_switch.conf を読み直して ENABLE_MODE_SWITCH を更新する"""
    global ENABLE_MODE_SWITCH, MODE_SWITCH_CONF
    MODE_SWITCH_CONF = str(conf_path(_STACKS_DIR))
    ENABLE_MODE_SWITCH = read_enable_mode_switch(_STACKS_DIR)
    return ENABLE_MODE_SWITCH
...
class SystemState:
    def __init__(self):
        ...
        self.startup_skips_remaining = 0  # 初動スキップ残り回数
...
def clear_ng_lock(reason="ブザー完了による自動復帰", expected_generation=None):
    ...
    # ライン再始動直後の1発目誤NGを避ける
    state.startup_skips_remaining = STARTUP_SKIP_COUNT
    update_status_leds()
    logging.info(f"【自動復帰】NGロックを解除。判定を再開します（{reason}）。")
    if STARTUP_SKIP_COUNT > 0:
        logging.info(f"【初動スキップ】復帰後の判定を {STARTUP_SKIP_COUNT} 回スキップします。")
...
def on_system_start():
    ...
    if not state.is_active:
        ...
        if STARTUP_SKIP_COUNT > 0:
            logging.info(
                f"【初動スキップ】開始直後の判定を {STARTUP_SKIP_COUNT} 回スキップします。"
            )
...
def sample_cmos_ok(sensor):
    """C-MOS を複数回読み、1回でも ON なら OK。全回 OFF のときだけ NG。"""
    count = max(1, int(CMOS_SAMPLE_COUNT))
    interval = max(0.0, float(CMOS_SAMPLE_INTERVAL))
...
def _judge_wafer_locked():
    ...
    # 初動スキップ（位相ずれ・センサ立ち上がり対策）
    if state.startup_skips_remaining > 0:
        state.startup_skips_remaining -= 1
        left = state.startup_skips_remaining
        logging.info(f"【初動スキップ】判定を見送りました（残り {left} 回）。")
        write_csv_log("枚数判定", "スキップ", f"初動スキップ（残り{left}）")
        return

    time.sleep(CHECK_DELAY)

    sensor = active_cmos()
    is_ok = sample_cmos_ok(sensor)
    ...
    if is_ok:
        ...
        reason = f"枚数異常/厚み規格外 ({sensor_label} OFF, {CMOS_SAMPLE_COUNT}回一致)"
        logging.warning(
...
def initialize_runtime():
    ...
    _ng_lock_generation = 0
    state.sheet_mode = 3
    state.startup_skips_remaining = 0
    ...
    bind_handlers()
    logging.info("==============================================")
    logging.info(" 枚数検知監視 プログラムVer 2.8.3 (初動スキップ + C-MOS複数回) ")
    logging.info(f" ENABLE_MODE_SWITCH={ENABLE_MODE_SWITCH} (conf: {MODE_SWITCH_CONF})")
    logging.info(f" STARTUP_SKIP_COUNT={STARTUP_SKIP_COUNT}, CMOS_SAMPLE_COUNT={CMOS_SAMPLE_COUNT}")
